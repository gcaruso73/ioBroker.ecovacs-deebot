...
async function handleStateChange(adapter, id, state) {
    ...
    if (channelName === 'map') {
        ...
        if (stateName === 'liveRobotOverlay') {
            adapter.vacbot.createMapImageOnPositionChange = !!(state && state.val);
            adapter.log.info('Live robot map overlay: ' + (adapter.vacbot.createMapImageOnPositionChange ? 'on' : 'off'));
            if (adapter.vacbot.createMapImageOnPositionChange && adapter.currentMapID
                && adapter.vacbot.is950type_V2 && adapter.vacbot.is950type_V2()) {
                // populate the room cache once so the overlay can render immediately
...
function cleanSpotArea(adapter, mapID, mapSpotAreaID) {
    if (adapter.getModel().isSupportedFeature('map.deebotPositionIsInvalid') && adapter.deebotPositionIsInvalid) {
        ...
        // Route through startSpotAreaCleaning so V2 devices use the V2/freeClean path
        startSpotAreaCleaning(adapter, mapSpotAreaID, adapter.spotAreaCleanings);
        if (adapter.spotAreaCleanings > 1) {
