...
const SUPPORTED_STATES = {
    ...
    "5xu9h3": {
        ...
        "info.dustbox": false,
        "map": true,
        "map.deebotPositionCurrentSpotAreaID": false,
        "map.deebotPositionIsInvalid": false,
        "map.lastUsedAreaValues": false,
        "map.spotAreas": false,
        "info.sleepStatus": true
...
class Model {
    ...
    isSupportedFeature(state) {
        if (this.getCanonicalDeviceClass() && this.config) {
            ...
            if (configOptionVal === '') {
                const classes = [this.getDeviceClass(), this.getCanonicalDeviceClass()];
        ...
        if (DEFAULT_VALUES.includes(state)) {
            return true;
        }
        return false;
    }
     ...
     * Delegates to the library's `deviceType` property (defined in `modelTypes.js`)
     * which is resolved via `vacbot.getDeviceProperty('deviceType')`.
     * @returns {string} User-friendly device type name (e.g. 'Vacuum Cleaner', 'Air Purifier')
     */
    getDeviceType() {
